...
import 'package:daily_globe_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template daily_globe_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class DailyGlobeApiMalformedResponse implements Exception {
  /// {@macro daily_globe_api_malformed_response}
  const DailyGlobeApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template daily_globe_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class DailyGlobeApiRequestFailure implements Exception {
  /// {@macro daily_globe_api_request_failure}
  const DailyGlobeApiRequestFailure({
...
/// {@template daily_globe_api_client}
/// A Dart API client for the Daily Globe API.
/// {@endtemplate}
class DailyGlobeApiClient {
  /// Create an instance of [DailyGlobeApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro daily_globe_api_client}
  DailyGlobeApiClient({
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw DailyGlobeApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        DailyGlobeApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
